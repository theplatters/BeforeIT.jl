function search_and_matching_credit!(world::Ark.World)
    ...
    @dub for row in Ark.Query(world, (LoanFlow,), with = (Firm,))        row.loan_flow.amount .= 0.0
    end
    ...
    n_active = 0
    @dub for row in Ark.Query(world, (TargetLoans,), with = (Firm,))        for i in eachindex(row.e)
