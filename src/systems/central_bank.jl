...
function set_central_bank_rate!(world)
    ...
    @dub for row in Ark.Query(world, (NominalInterestRate,), with = (CentralBank,))
        @inbounds for i in eachindex(row.e)
            row.nominal_interest_rate[i] = (
                taylor_rule(
                    interest_rate_smoothing,
                    row.nominal_interest_rate[i].rate,
                    natural_rate,
                    inflation_target,
...
function set_central_bank_equity!(world)
    ...
    @dub for row in Ark.Query(world, (Equity, NominalInterestRate), with = (CentralBank,))
