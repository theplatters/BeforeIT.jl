function set_bank_deposits!(world::Ark.World)
    ...
    @dub for row in Ark.Query(world, (Equity, ResidualItems), with = (Bank,))        for i in eachindex(row.e)
...
function finance_insolvent_firms!(world::Ark.World)
    ...
    @dub for row in Ark.Query(world, (LoansOutstanding, Equity, Deposits, CapitalStock))        for i in eachindex(row.e)
    ...
    @dub for row in Ark.Query(world, (Equity,), with = (Bank,))        row.equity.amount .-= financed_total_equity
    end
...
function set_bank_expected_profits!(world)
    ...
    @dub for row in Ark.Query(world, (ExpectedProfits, Profits), with = (LendingRate,))        row.expected_profits.amount .= row.profits.amount .* (1 + output_growth) .* (1 + inflation)
    end
...
function set_bank_rate!(world)
    cb_rate = 0.0
    @dub for row in Ark.Query(world, (NominalInterestRate,))        for i in eachindex(row.e)
    ...
    @dub for row in Ark.Query(world, (LendingRate,))        row.lending_rate.rate .= cb_rate + mu
    end
...
function set_bank_equity!(world::Ark.World)
    ...
    @dub for row in Ark.Query(world, (Equity, Profits), with = (Bank,))        row.equity.amount .= row.equity.amount .+
            row.profits.amount .-
...
function set_bank_profits!(world)
    ...
    @dub for row in Ark.Query(world, (Profits, LendingRate, ResidualItems))        @inbounds for i in eachindex(row.e)
