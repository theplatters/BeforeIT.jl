function set_bank_deposits!(world::Ark.World)
    ...
    @dub for row in Ark.Query(world, (Equity, ResidualItems), with = (Bank,))
...
function finance_insolvent_firms!(world::Ark.World)
    ...
    @dub for row in Ark.Query(world, (LoansOutstanding, Equity, Deposits, CapitalStock))
        for i in eachindex(row.e)
    ...
    @dub for row in Ark.Query(world, (Equity,), with = (Bank,))
...
function set_bank_expected_profits!(world)
    ...
    @dub for row in Ark.Query(world, (ExpectedProfits, Profits), with = (LendingRate,))
...
function set_bank_rate!(world)
    ...
    @dub for row in Ark.Query(world, (LendingRate,))
...
function set_bank_equity!(world::Ark.World)
    ...
    @dub for row in Ark.Query(world, (Equity, Profits), with = (Bank,))
...
function set_bank_profits!(world)
    ...
    @dub for row in Ark.Query(world, (Profits, LendingRate, ResidualItems))
